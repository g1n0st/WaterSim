...
class MGPCGPoissonSolver:
    def __init__(self, dim, res, n_mg_levels = 4, pre_and_post_smoothing = 2, bottom_smoothing = 50, real = float):
        ...
        # grid type
        self.grid_type = [ti.field(dtype=ti.i16) 
                        for _ in range(self.n_mg_levels)]
        self.boundary = [ti.field(dtype=ti.i16) 
                        for _ in range(self.n_mg_levels)]
    ...
    def init_gridtype(self, grid0 : ti.template(), grid : ti.template(), boundary : ti.template()):
        for I in ti.grouped(grid):
            ...
            in_D = 0 # D(P,I(l)) as the set of coarse cells in the prolongation stencil of an interior cell I(l)
            for offset in ti.static(ti.grouped(ti.ndrange(*((-1, 3), ) * self.dim))):
                in_D |= (grid0[I2 + offset] == self.FLUID)

            # if any of its 8 fine children are not inte- rior cells, I(l) is included in the boundary band
            if in_D and tot_fluid < (2**self.dim):
                boundary[I] = 1
    ...
    def v_cycle(self):
        ...
        for i in range(self.bottom_smoothing):
            # self.boundary_smooth(self.n_mg_levels - 1, 0)
            # self.boundary_smooth(self.n_mg_levels - 1, 1)
            self.smooth(l)
